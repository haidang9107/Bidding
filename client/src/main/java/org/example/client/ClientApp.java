 ...
 * Updated for the 'Major Surgery' (accountname as PK) and new DTOs.
 */
public class ClientApp {
    ...
    public static void main(String[] args) {
        try {
            ...
            client.sendRequest(new Request(MessageType.ADMIN_BAN_USER, new AdminUserControlRequest("huong_member", 0)));
            waitForResponse();
            ...
            System.out.println("-> Fetching auctions with PAGINATION (Page: 1, Size: 5)...");
            client.sendRequest(new Request(MessageType.PRODUCT_LIST, new PaginationRequest(1, 5)));
            waitForResponse();

            System.out.println("-> Fetching details for Product ID: 1...");
            client.sendRequest(new Request(MessageType.PRODUCT_DETAIL, 1));
            waitForResponse();

            // --- SCENARIO 6: Profile Features ---
            testHeader("SCENARIO 6: Profile Features");
            System.out.println("-> Fetching profile for 'nam_member'...");
            client.sendRequest(new Request(MessageType.GET_PROFILE, null));
            waitForResponse();

            System.out.println("-> Updating Profile (Avatar & Email) for 'nam_member'...");
            client.sendRequest(new Request(MessageType.UPDATE_PROFILE, new UserProfileUpdateRequest("nam.new@gmail.com", "https://api.dicebear.com/7.x/avataaars/svg?seed=nam_updated")));
            waitForResponse();

            // --- SCENARIO 7: RBAC (Role-Based Access Control) ---
            testHeader("SCENARIO 7: Role-Based Access Control (RBAC)");
            System.out.println("-> Member trying to ADD_PRODUCT (Should be SUCCESS)...");
            ...
            System.out.println("-> Admin trying to ADD_PRODUCT (Should be FORBIDDEN)...");
            ...
            // --- SCENARIO 9: Admin Controls ---
            testHeader("SCENARIO 9: Admin Controls");
            System.out.println("-> Switching back to Admin...");
            client.sendRequest(new Request(MessageType.LOGOUT, null));
            waitForResponse();
            client.sendRequest(new Request(MessageType.LOGIN, new LoginRequest("admin_minh", "password123")));
            waitForResponse();

            // --- SCENARIO 10: Admin User List Pagination ---
            testHeader("SCENARIO 10: Admin User List Pagination");
            System.out.println("-> Fetching users with PAGINATION (Page: 1, Size: 2)...");
            client.sendRequest(new Request(MessageType.ADMIN_GET_ALL_USERS, new PaginationRequest(1, 2)));
            waitForResponse();
            
            System.out.println("-> Admin canceling an auction (Product ID: 1)...");
            client.sendRequest(new Request(MessageType.ADMIN_CANCEL_AUCTION, new AuctionCancelRequest(1, "Test cancellation")));
            waitForResponse();

            System.out.println("-> Admin banning 'huong_member'...");
            client.sendRequest(new Request(MessageType.ADMIN_BAN_USER, new AdminUserControlRequest("huong_member", 1)));
            waitForResponse();
